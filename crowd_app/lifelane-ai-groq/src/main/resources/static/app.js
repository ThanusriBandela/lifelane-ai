/* ═══════════════════════════════════════════════════════════════
   LifeLane AI — app.js  [UPGRADED v2]
   ✔ All previous features (routing, AI, IoT, WebSocket, maps)
   ✔ RFID + Voice input + Voice broadcast
   ── NEW UPGRADES ──────────────────────────────────────────────
   ✅ [1] Auto Voice  — mic starts automatically in emergency
   ✅ [2] Live Vehicle Movement — animated ambulance + trail + stats
   ✅ [3] AI Traffic Prediction — T+5/10/15 min forecasting
   ✅ [4] Nearby User Alerts — popup "🚨 Ambulance approaching"
   ✅ [5] Offline Mode — cached route + voice warning
   ═══════════════════════════════════════════════════════════════ */
'use strict';
...
function goToHome()   { showPage('page-home');   disconnectWS(); stopVoiceAlert(); }
...
function initMapPage(mode) {
  ...
  // Crash panel: only in normal mode, visibility managed by checkCrashPanelVisibility()
  const crashPanel = document.getElementById('crash-alert-panel');
  if (crashPanel) crashPanel.style.display = (mode === 'normal') ? 'none' : 'none'; // starts hidden, shown after src+dst set
  if (!map) {
...
function registerUser() {
  ...
  const c1phone  = document.getElementById("contact1-phone").value.trim();
  const c2name   = document.getElementById("contact2-name").value.trim();
  const c2phone  = document.getElementById("contact2-phone").value.trim();

  if (!name || !email || !password) {
    showToast('warning', 'Please fill in name, email and password');
    return;
  }

  const user = { name, email, password, contact1Name: c1name, contact1Phone: c1phone, contact2Name: c2name, contact2Phone: c2phone };
  const accounts = JSON.parse(localStorage.getItem('lifelane-accounts') || '{}');
  if (accounts[email]) { showToast('warning', 'Email already registered. Please sign in.'); return; }
  accounts[email] = user;
  localStorage.setItem('lifelane-accounts', JSON.stringify(accounts));
  showToast('success', 'Account created! Please sign in.');
  showPage("page-login");

  // Also try saving to backend (non-blocking)
  fetch(API_BASE + '/api/contacts', {
    method: 'POST', headers: {'Content-Type':'application/json'},
    body: JSON.stringify({ driverName: email, name: c1name || 'Contact 1', phone: c1phone || '' })
  }).catch(()=>{});
  if (c2name) fetch(API_BASE + '/api/contacts', {
    method: 'POST', headers: {'Content-Type':'application/json'},
    body: JSON.stringify({ driverName: email, name: c2name, phone: c2phone || '' })
  }).catch(()=>{});
}
...
function loginUser() {
  ...
  showToast('success', 'Welcome back, ' + (user.name ? user.name.split(' ')[0] : 'User') + '!');
...
function showTrafficPredictionPanel(routes) {
  ...
  if(willCrowd){showToast('warning','⚠️ AI: Road will crowd in ~5 min');addLog('warning','AI PREDICT','Traffic surge predicted — consider alternate route');}
  ...
  panel.innerHTML=`
    ...
    ${willCrowd?'<div class="pred-warning">⚠️ Congestion spike predicted — alternate route recommended!</div>':'<div class="pred-ok">✅ Route looks clear for the next 15 minutes</div>'}
  `;
}
...
async function calculateRoute() {
  ...
  if(S.isOffline){
    if(S.lastSavedRoute){showToast('warning','⚠️ Offline — using last saved route');addLog('warning','OFFLINE','No internet — showing cached route');speak('You are offline. Using last saved route.');redrawCachedRoute();return;}
    else{showToast('warning','⚠️ Offline — no cached route available');speak('You are offline and no cached route is available.');return;}
  }
  ...
  finally{btn.disabled=false;btnTxt.textContent=S.mode==='emergency'?'🚨 DISPATCH EMERGENCY':'🗺️ FIND ROUTES';}
}
...
function initOfflineMonitor() {
  ...
  window.addEventListener('offline',()=>{S.isOffline=true;updateOfflineBanner();showToast('warning','⚠️ You are offline');addLog('warning','OFFLINE','Internet lost — cached route available');speak('Warning: You are offline. Last saved route is available.');if(S.lastSavedRoute&&!S.vehicleInterval)redrawCachedRoute();});
}
function updateOfflineBanner() {
  ...
  if(S.isOffline){banner.innerHTML='⚠️ OFFLINE — Using last saved route';banner.style.display='flex';}else{banner.style.display='none';}
}
function redrawCachedRoute() {
  ...
  try{const lbl=L.tooltip({permanent:true,direction:'center',className:'offline-route-label'}).setContent('⚠️ CACHED ROUTE (OFFLINE)').setLatLng(coords[Math.floor(coords.length/2)]);map.addLayer(lbl);S.routeLayers.push(lbl);}catch(e){}
  map.fitBounds(L.latLngBounds(coords),{padding:[40,40]});
  addLog('warning','OFFLINE','Cached route displayed — You are offline. Using last route…');
  showToast('warning','🗺️ Offline: Using last saved route');
...
function clearAll() {
  ...
  const cp = document.getElementById('crash-alert-panel'); if(cp) cp.style.display='none';
...
async function loadDispatchHistory() {
  ...
  try {
    ...
    list.innerHTML = '<div class="history-empty">⚠️ Could not load history.<br>Make sure the backend is running.</div>';
  }
}

function vehicleIcon(type) {
  const icons = { AMBULANCE:'🚑', FIRE_TRUCK:'🚒', POLICE:'🚓', HAZMAT:'☣️', NORMAL:'🗺️' };
  return icons[type] || '🚗';
}
...
/* ══════════════════════════════════════════════════════════════
   CRASH ALERT MODULE — manual button, normal routing mode only
══════════════════════════════════════════════════════════════ */

function checkCrashPanelVisibility() {
  const panel = document.getElementById('crash-alert-panel');
  if (!panel) return;
  // Show only in normal mode when both src and dst are set
  if (S.mode === 'normal' && S.src && S.dst) {
...
function renderCrashContactsPreview() {
  ...
  if (user.contact1Name || user.contact1Phone) {
    html += `<span>👤 ${user.contact1Name || 'Contact 1'}${user.contact1Phone ? ' · ' + user.contact1Phone : ''}</span>`;
    hasContact = true;
  }
  if (user.contact2Name || user.contact2Phone) {
    html += `<span>👤 ${user.contact2Name || 'Contact 2'}${user.contact2Phone ? ' · ' + user.contact2Phone : ''}</span>`;
    hasContact = true;
  }
...
function triggerCrashAlert() {
  const user = JSON.parse(localStorage.getItem('user') || 'null');
  if (!user) { showToast('warning', 'Please sign in first to use crash alerts'); return; }
  if (!S.src || !S.dst) { showToast('warning', 'Set source and destination first'); return; }

  // Populate modal with route + contact info
  const routeDiv = document.getElementById('crash-modal-route');
  if (routeDiv) {
    routeDiv.innerHTML =
      `<div>📍 <b>From:</b> ${S.src.name}</div>` +
      `<div>🏁 <b>To:</b> ${S.dst.name}</div>` +
      `<div style="margin-top:4px;opacity:0.6">📡 GPS: ${S.src.lat.toFixed(5)}, ${S.src.lng.toFixed(5)}</div>`;
  }
  ...
  if (contactsDiv) {
    let chips = '';
    if (user.contact1Name || user.contact1Phone)
      chips += `<div class="contact-chip">👤 ${user.contact1Name || 'Contact 1'}${user.contact1Phone ? ' · ' + user.contact1Phone : ''}</div>`;
    if (user.contact2Name || user.contact2Phone)
      chips += `<div class="contact-chip">👤 ${user.contact2Name || 'Contact 2'}${user.contact2Phone ? ' · ' + user.contact2Phone : ''}</div>`;
    if (!chips) chips = '<div class="contact-chip" style="opacity:0.5">⚠ No contacts saved</div>';
...
async function confirmCrashAlert() {
  ...
  // Get current GPS if available, else use src coords
  ...
  } catch(e) { /* use route src coords */ }

  const mapsUrl = `https://maps.google.com/?q=${lat},${lng}`;
  const message = `🚨 CRASH ALERT from ${user.name || 'LifeLane User'}!
📍 From: ${S.src ? S.src.name : 'Unknown'}
🏁 To: ${S.dst ? S.dst.name : 'Unknown'}
📡 Location: ${lat.toFixed(5)}, ${lng.toFixed(5)}
🗺 Maps: ${mapsUrl}

Please respond immediately!`;

  // Try backend alert endpoint
  let backendSent = false;
  try {
    const driverKey = user.email || 'driver1';
    const res = await fetch(API_BASE + `/api/contacts/alert?driver=${encodeURIComponent(driverKey)}&lat=${lat}&lng=${lng}`, {
      method: 'POST'
    });
    if (res.ok) backendSent = true;
  } catch(e) { /* offline — fall through */ }

  // Build sent message
  let notified = [];
  if (user.contact1Name || user.contact1Phone) notified.push(user.contact1Name || user.contact1Phone);
  if (user.contact2Name || user.contact2Phone) notified.push(user.contact2Name || user.contact2Phone);
  const notifiedStr = notified.length ? notified.join(', ') : 'your emergency contacts';

  // Log to alert feed
  addLog('critical', 'CRASH ALERT', `🚨 Alert sent to ${notifiedStr} · Location: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);

  // Show sent overlay
  const overlay = document.getElementById('crash-sent-overlay');
  const sentMsg = document.getElementById('crash-sent-msg');
  if (sentMsg) sentMsg.innerHTML = `Alert sent to: ${notifiedStr}<br>Location: ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
  if (overlay) {
  ...
  showToast('critical', `🚨 Crash alert sent to ${notifiedStr}`);
...
function autoSendCrashAlert() {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
        ...
        fetch(API_BASE + `/api/contacts/alert?driver=driver1&lat=${lat}&lng=${lng}`, { method: 'POST' })
            .then(r => r.json())
            .then(data => {
